...
import { X, Send, AlertTriangle, CheckCircle, Store, MapPin, DollarSign } from 'lucide-react';
import { STORES_LIST } from '../data/mockProducts';
...
export const ReportPriceModal: React.FC<ReportPriceModalProps> = ({ onClose, selectedCity }) => {
  const [productName, setProductName] = useState('');
  const [storeName, setStoreName] = useState(STORES_LIST[0].name);
  const [reportedPrice, setReportedPrice] = useState('');
  const [reportType, setReportType] = useState<'oferta' | 'sobreprecio' | 'actualizacion'>('oferta');
        ...
        {submitted ? (
          <div className="p-8 text-center space-y-3">
            ...
            <p className="text-sm text-stone-500 dark:text-stone-400">
              Tu reporte fue registrado y será auditado para actualizar el comparador.
            </p>
          </div>
        ) : (
